...
- Max width: 1920px
- JPEG quality: ~0.8
- Target: under 3MB for images
- Uses `<canvas>` for resizing and re-encoding.
...
- **JPEG/PNG/WebP:** Upload original file. Validate size against `max_file_size` (15MB).
- **HEIC:** Convert to JPEG client-side using `heic2any` (browsers cannot display HEIC natively). Upload as JPEG.
- **RAW (CR2, NEF, ARW):**
...
async function prepareUpload(file, config) {
  ...
  // 1. Handle HEIC (always convert to JPEG for compatibility)
  if (file.name.toLowerCase().endsWith('.heic')) {
    const converted = await heic2any({ blob: file, toType: 'image/jpeg', quality: 0.9 });
    uploadFile = Array.isArray(converted) ? converted[0] : converted; // Handle multi-image HEIC
  }

  // 2. Generate Thumbnail (if possible)
  if (isRawFormat(uploadFile.name)) {
    // RAW file: use static placeholder, server will fix
    thumbnailBlob = await fetch('/assets/placeholder-raw.jpg').then(r => r.blob());
  } else {
    // Standard image (JPEG/PNG/WebP): generate client-side
    thumbnailBlob = await generateThumbnail(uploadFile);
  }

  // 3. Compression Logic
  if (config.compression_mode === 'raw') {
    if (uploadFile.size > config.max_file_size) throw new Error('File too large');
    return { original: uploadFile, thumbnail: thumbnailBlob };
  }
  
  // Default: compressed mode
  if (isRawFormat(uploadFile.name)) throw new Error('RAW files only allowed in Uncompressed Mode');
  
  const compressed = await compressImage(uploadFile, { maxWidth: 1920, quality: 0.8 });
  return { original: compressed, thumbnail: thumbnailBlob };
}

function isRawFormat(filename) {
  return /\.(cr2|nef|arw|dng|orf|rw2)$/i.test(filename);
}
```
```
...
| Client compression    | Required. Max 1920px, JPEG 0.8| None — original uploaded      |
